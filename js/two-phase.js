 ...
 *
 * The methods humans memorise are built to be memorable, not short — a beginner
 * layer-by-layer solve runs to ~130 moves. This searches instead, and lands
 * around 20, which is what the "shortest" mode shows.
 *
 ...
 * cube.js, so the cube on screen and the cube being searched can never disagree
 * about what "R" means.
 *
 * Depends on (loaded before this file):
 *   window.Cube    (cube.js)
 *   window.Cubie   (cubie.js)
 */
...
(function () {
  ...
  function init() {
    if (ready) return;
    if (typeof Cube === 'undefined') throw new Error('cube.js must be loaded first');
    ...
    for (const name of MOVE_NAMES) {
      const c = new Cube();
      c.applyMove(name);
      MOVES.push(Cubie.fromFacelets(c.getState()));
    }
  ...
  window.TwoPhaseSolver = {
    ...
    /**
     * Solve a facelet state (the shape Cube.getState() returns).
     * @param {object} state
    ...
    solve(state, opts) {
      init();
      const found = solveCubie(Cubie.fromFacelets(state), opts);
